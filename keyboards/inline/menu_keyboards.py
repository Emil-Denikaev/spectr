...
async def categories_keyboard():
    ...
    for category in categories:
        ...
        button_text = f"{category.category_name} ({number_of_items} штук)"
...
async def subcategories_keyboard(category):
    ...
    for subcategory in subcategories:
        ...
        button_text = f"{subcategory.subcategory_name} ({number_of_items} штук)"
...
async def items_keyboard(category, subcategory):
    ...
    for item in items:
        button_text = f"{item.name} - {item.price} рублей"
...
def item_keyboard(category, subcategory, item_id):
    ...
    markup = InlineKeyboardMarkup()
    markup.row(
        InlineKeyboardButton(text=f"Купить", callback_data=buy_item.new(item_id=item_id))
    )
    markup.row(
